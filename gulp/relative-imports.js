...
const CORE_PACKAGE_NAME = 'ng2-swipe-cards';
const CORE_IMPORT_REGEX = /^(.*\.\.?\/components\/ng2-swipe-cards)(.*)$/;
...
function tsDefinitionImportRename() {
  const CORE_IMPORT_REGEX = /^(import|export)( \{.+\} from ['"])(.*\/components\/ng2-swipe-cards)(['"];?)$/mg;
  return replace(CORE_IMPORT_REGEX, `$1$2${CORE_PACKAGE_NAME}$4`);
}

function ngcMetadataRename() {
  const CORE_IMPORT_REGEX = /(\.\..*?\/components\/ng2-swipe-cards)/g;
